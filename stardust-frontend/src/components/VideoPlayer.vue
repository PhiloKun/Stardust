<template>
  <div class="video-player-component" ref="playerContainer" tabindex="0">
    ...
    <!-- 全局进度条，固定在底部 -->
    <div class="video-progress-container" v-show="showControls" @click.stop="seekVideo($event, currentVideoIndex)" @touchstart.stop="startDrag($event)" @touchmove.stop="onDrag($event)" @touchend.stop="endDrag()">
    ...
    <!-- 电脑端操作指引 -->
    <div class="pc-controls-guide" v-if="isPcDevice && showControls">
      <div class="guide-item">
        <van-icon name="arrow-up" /> <van-icon name="arrow-down" />
        <span>键盘⬆️⬇️或鼠标滚轮</span>
      </div>
      <div class="guide-item">
...
<script setup>
import { ref, onMounted, reactive, onBeforeUnmount, computed, inject } from 'vue';
...
const videoList = ref([
  {
    videoUrl: 'https://lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/byted-player-videos/1.0.0/xgplayer-demo-360p.mp4',
  ...
  {
    videoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-countryside-meadow-4075-large.mp4',
...
onBeforeUnmount(() => {
  ...
  if (playerContainer.value) {
    ...
    playerContainer.value.removeEventListener('touchstart', showControlsTemporarily);
  }
...
.video-player-component {  
  ...
  width: 100%;  
  height: calc(100% - 46px - 50px); /* 减去顶部导航栏和底部导航栏的高度 */  
  margin: 0;  
  border-radius: 0;  
  overflow: hidden;  
  background-color: #000;  
  z-index: 1; /* 降低z-index，确保不会覆盖导航栏 */
  outline: none; /* 去除focus时的边框 */
}
