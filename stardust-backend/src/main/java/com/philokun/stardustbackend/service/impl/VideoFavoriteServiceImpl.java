...
import com.philokun.stardustbackend.mapper.VideoFavoriteMapper;
...
@Service
public class VideoFavoriteServiceImpl extends ServiceImpl<VideoFavoriteMapper, VideoFavorite>
    ...
    public boolean unfavorite(String userId, String videoId) {
        return this.remove(new QueryWrapper<VideoFavorite>().eq("user_id", userId).eq("video_id", videoId));
    }

    @Override
    public boolean hasFavorited(String userId, String videoId) {
        return this.count(new QueryWrapper<VideoFavorite>().eq("user_id", userId).eq("video_id", videoId)) > 0;
    }

    @Override
    public List<String> listFavoriteVideoIds(String userId) {
        return this.lambdaQuery().eq(VideoFavorite::getUserId, userId)
                .list().stream().map(VideoFavorite::getVideoId).toList();
    }
}